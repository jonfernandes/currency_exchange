...
if st.button("Submit"):
    if user_input:
        ...
        print(completion)
        if completion['choices'][0]['message']['finish_reason'] == "tool_calls":
        ...
        elif completion['choices'][0]['message']['finish_reason'] == "stop":
            st.write(f"(Function calling not used): {completion}")
        else:
